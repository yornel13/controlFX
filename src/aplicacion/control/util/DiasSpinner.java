 ...
 * @author Thomas Bolz
 */
public class DiasSpinner extends HBox {
    ...
    public DiasSpinner(BigDecimal value, BigDecimal stepWidth) {
        this(value, stepWidth, NumberFormat.getInstance());
    }

    public DiasSpinner(BigDecimal value, BigDecimal stepWidth, NumberFormat nf) {
        super();
    ...
    private void increment() {
        String[] splited = numberField.getText().split(" ");
        BigDecimal value = new BigDecimal(splited[0]);
        value = value.add(stepWitdhProperty.get());
        if (value.equals(new BigDecimal(1)))
            numberField.setText("1 dia"); 
        else
            numberField.setText(value + " dias");
    }
